    ...
    <div className="space-y-4">
      ...
      {questions.map((q, idx) => {
        ...
        return (
          <div
            key={q.id}
            className={`border rounded-2xl overflow-hidden p-4 sm:p-5 space-y-4 transition-all ${theme.cardContainer} ${
              isCorrect
          ...
          >
            ...
            <div className="space-y-2">
              <div lang="ja" className={`text-lg font-semibold tracking-wide font-sans leading-relaxed py-3 rounded-xl px-4 select-all border ${theme.wordPanelBg} ${theme.tableBorder} ${theme.breakdownKanjiMeaning}`}>
                {parts.map((p, pIdx) => {
                  if (pIdx % 2 === 1) {
                    if (p.toLowerCase() === "blank") {
                      // 빈칸 자리에 정답 단어를 밀어넣어 완전한 문장으로 표시
                      return (
                        <strong key={pIdx} className="text-emerald-500 font-extrabold underline underline-offset-4 decoration-emerald-500 mx-1">
                          {q.targetWord}
                        </strong>
                      );
                    } else {
                      // 키워드 강조 처리
                      return (
                        <strong key={pIdx} className="text-amber-500 font-extrabold mx-0.5">
                          {p}
                        </strong>
                      );
                    }
              ...
              {/* 번역 한글 해석 제공 */}
              <p className={`text-xs font-medium italic ${theme.wordSubText}`}>
                * 해석 : {q.translation}
              </p>
            </div>
            ...
            <div className="space-y-1">
              <h5 className={`text-sm font-bold ${theme.breakdownKanjiMeaning}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              ...
              <div className={`p-3 rounded-xl space-y-0.5 border ${theme.wordPanelBg} ${theme.tableBorder}`}>
                ...
                <span lang="ja" className="font-bold text-emerald-500">
              ...
              <div className={`p-3 rounded-xl space-y-0.5 border ${
                ...
                <span className={`font-medium block ${theme.wordSubText}`}>내가 고른 답</span>
                <span lang="ja" className={`font-semibold ${isCorrect ? theme.breakdownKanjiMeaning : "text-rose-500 font-bold"}`}>
                  {ansIdx !== undefined
                    ? q.choices[ansIdx]
                    : "응답 없음"}
                </span>
